...
UV_CMD=""
if is_termux; then
  echo -e "${CYAN}→${NC} Termux detected — using Python's stdlib $PYTHON_VENV_NAME + pip instead of uv"
else
...
if [ -d "$PYTHON_VENV_NAME" ]; then
  echo -e "${CYAN}→${NC} Removing old $PYTHON_VENV_NAME..."
  rm -rf "$PYTHON_VENV_NAME"
fi

if is_termux; then
  "$PYTHON_PATH" -m venv $PYTHON_VENV_NAME
  echo -e "${GREEN}✓${NC} venv created with stdlib venv"
else
  $UV_CMD venv $PYTHON_VENV_NAME --python "$PYTHON_VERSION"
  echo -e "${GREEN}✓${NC} $PYTHON_VENV_NAME created (Python $PYTHON_VERSION)"
fi

export VIRTUAL_ENV="$SCRIPT_DIR/$PYTHON_VENV_NAME"
SETUP_PYTHON="$SCRIPT_DIR/$PYTHON_VENV_NAME/bin/python"
...
if is_termux; then
  ...
  if [ -f "uv.lock" ]; then
    ...
    if UV_PROJECT_ENVIRONMENT="$SCRIPT_DIR/$PYTHON_VENV_NAME" $UV_CMD sync --extra all --locked; then
...
HERMES_BIN="$SCRIPT_DIR/$PYTHON_VENV_NAME/bin/hermes"
...
if "$SCRIPT_DIR/$PYTHON_VENV_NAME/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
  ...
  "$SCRIPT_DIR/$PYTHON_VENV_NAME/bin/python" -m hermes_cli.main setup
fi
